...
$dest_dossier = '../../image/'; //nom du dossier ou vous allez stocké vos images
$dest_fichier="";

//**************************************
if(!$getimagesize = getimagesize($_FILES['ArtImage']['tmp_name'])) {
$erreurs[] = "Le fichier n'est pas une image valide.";
}
//*******************************
else {
if( (!in_array( get_extension($_FILES['ArtImage']['name']), $extensions_ok ))or (!in_array($getimagesize[2], $typeimages_ok )))
{
...
if( file_exists($_FILES['ArtImage']['tmp_name']) and filesize($_FILES['ArtImage']['tmp_name']) > $taille_max)
{
$erreurs[] = "Votre fichier doit faire moins de $taille_ko Ko !";
}
else{

$dest_fichier = basename($_FILES['ArtImage']['name']);
...
//}
//********************************
if(move_uploaded_file($_FILES['ArtImage']['tmp_name'], $dossier . $dest_fichier)) {
